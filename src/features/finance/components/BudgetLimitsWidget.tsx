...
import { budgetLimitsDbService, type BudgetLimit } from '../budgetLimitsDbService';
...
export function BudgetLimitsWidget({ transactions }: BudgetLimitsWidgetProps) {
  ...
  useEffect(() => {
    budgetLimitsDbService.getAll()
      .then(setLimits)
      .catch(err => console.error('[BudgetLimits] load error', err))
  ...
  const addLimit = async () => {
    ...
    try {
      await budgetLimitsDbService.upsert(newCategory, Number(newLimit));
      const updated = await budgetLimitsDbService.getAll();
      setLimits(updated);
      setNewLimit('');
      setShowAdd(false);
  ...
  const removeLimit = async (category: string) => {
    try {
      await budgetLimitsDbService.remove(category);
    ...
    return (
      <div className="bg-card border border-border rounded-xl p-5">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <Target className="w-4 h-4" /> Loading budget limits…
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-5 space-y-4">
      ...
      {showAdd && (
        <div className="flex items-center gap-2 p-3 bg-muted/30 rounded-lg">
          ...
          <input type="number" value={newLimit} onChange={e => setNewLimit(e.target.value)}
            ...
            onKeyDown={e => { if (e.key === 'Enter') void addLimit(); }} />
          <button onClick={() => void addLimit()} className="p-1.5 rounded bg-primary text-primary-foreground hover:bg-primary/90">
      ...
      <div className="space-y-3">
        {limits.map(({ category, limitAmount }) => {
            ...
            <div key={category} className="space-y-1">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className={cn(
                  ...
                  €{spent.toFixed(0)} / €{limitAmount}
                </span>
                <button onClick={() => void removeLimit(category)}
